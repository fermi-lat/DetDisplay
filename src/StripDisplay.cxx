...
*
*  $Header: /nfs/slac/g/glast/ground/cvs/Recon/src/StripDisplay.cxx,v 1.2 2003/07/03 23:48:27 burnett Exp $
*/
        ...
        for( SiDetector::const_iterator strip = d.begin(); strip != d.end(); ++strip) {
            if (strip->noise() == m_noise) // select noise or not
            {
                float h = strip->energy() * pScale,
                    x0 = SiliconPlaneGeometry::localX(strip->index()) + strip->getDeltaX(),
                    y0 = SiliconPlaneGeometry::panel_width()/2 + strip->getDeltaY();
                h *= 1.;
...
void StripDisplay::displayStrips(const Event::McTkrStripCol& strips)
{
    ...
    for(SiDetMap::iterator im = m_SiMap.begin(); im != m_SiMap.end(); ++im){
        ...
        //  form a plane tranformation by averaging the offsers for  wafers 0,0 and 3,3
        idents::VolumeIdentifier   wafer00_id=plane_id, wafer33_id=plane_id;
        wafer00_id.append(0); wafer00_id.append(0);
        wafer33_id.append(3); wafer33_id.append(3);
        HepTransform3D T00, T33;
        m_detSvc->getTransform3DByID(wafer00_id, &T00);
        m_detSvc->getTransform3DByID(wafer33_id, &T33);
        Hep3Vector offset = 0.5*(T00.getTranslation() + T33.getTranslation());
        CoordTransform  T_plane(T00.getRotation(), offset);
